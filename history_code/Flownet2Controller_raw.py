...
class FlowController:
    ...
    @staticmethod
    def preprocess_frames_wzg(frame1, frame2,flow_height,flow_width):
        """
        图像预处理
        1. 确定尺寸相同
        2. 图像resize
        3. 调整通道顺序并扩展batch维度
        4. 转换成torch.tensor数据格式
        """
        assert frame1.shape == frame2.shape, "Shapes of both frames must be same"
        ...
        # frame.shape = [H,W,C]
        # process_resolution = tuple([max(64 * (frame1.shape[i] // 64), 64) for i in range(2)])
        process_resolution = (flow_height,flow_width)
        # 图像尺寸需要是64的倍数
        images = [cv2.resize(frame1, process_resolution), cv2.resize(frame2, process_resolution)] # [L,H,W,C]-->[C,L,H,W]
        images = np.expand_dims(np.array(images).transpose(3, 0, 1, 2), axis=0) # 增加batch维度--》[B,C,len,H,W]
        images = torch.from_numpy(images.astype(np.float32))
    ...
    def predict(self, image1, image2):
        ...
        (data, target) = self.preprocess_frames_wzg(image1, image2,384,512)
        if args.cuda:
        ...
        # data[0].shape =  [1, 3, 2, 512, 384]
        # output.shape = [1, 2, 512, 384]
        # resize 到原始图像尺寸
